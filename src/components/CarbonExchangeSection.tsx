      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <ScrollReveal direction="up">
          <div className="text-center mb-14">
            ...
            <h2 className="text-3xl sm:text-4xl font-extrabold text-[#14532D] mb-4">Green Mark Carbon Exchange</h2>
            <p className="text-[#14532D] text-lg max-w-2xl mx-auto">A transparent marketplace connecting smallholder farmers with corporates — enabling verified carbon credit trading for a sustainable India.</p>
          </div>
        </ScrollReveal>
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-center mb-14">
          <ScrollReveal direction="left" delay={0}>
            <div className="bg-[#14532D] rounded-2xl p-8 text-white shadow-xl h-full">
              ...
              <ul className="space-y-3">
                {['Generate carbon credits', 'Receive fair market value', 'Access global carbon markets', 'Monthly income stream'].map((item) => (
                  <li key={item} className="flex items-center gap-3 text-[#D6E8D2] text-sm">
                    <span className="w-1.5 h-1.5 bg-[#3A8A5C] rounded-full flex-shrink-0"></span>
                    {item}
                  </li>
                ))}
              </ul>
          ...
          <ScrollReveal direction="scale" delay={100}>
            <div className="text-center relative">
              <div className="bg-[#F7EDE2] border-2 border-[#D6E8D2] rounded-2xl p-8 relative shadow-lg hover:shadow-2xl transition-shadow duration-500">
                <div className="w-20 h-20 bg-gradient-to-br from-[#14532D] to-[#14532D] rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg">
                  <span className="text-3xl">♻️</span>
                </div>
                <h3 className="font-bold text-[#14532D] text-lg mb-1">MY GREEN MARK</h3>
                <h4 className="font-semibold text-[#3A8A5C] text-base mb-5">CARBON EXCHANGE</h4>
                <div className="flex flex-col gap-2 text-sm text-[#14532D]">
                  {['Aggregation', 'Verification', 'Trading', 'Settlement'].map((step) => (
                    <span key={step} className="bg-[#D6E8D2] px-3 py-1.5 rounded-full font-semibold hover:bg-[#D6E8D2] transition-colors cursor-default">{step}</span>
                  ))}
                </div>
            ...
            <div className="bg-[#14532D] rounded-2xl p-8 text-white shadow-xl h-full">
              ...
              <ul className="space-y-3">
                {['Buy SEBI-verified offsets', 'Meet net-zero ESG targets', 'CSR & BRSR compliance', 'Transparent audit trail'].map((item) => (
                  <li key={item} className="flex items-center gap-3 text-[#D6E8D2] text-sm">
                    <span className="w-1.5 h-1.5 bg-[#F4B942] rounded-full flex-shrink-0"></span>
                    {item}
                  </li>
                ))}
              </ul>
          ...
          {[
            ...
            <ScrollReveal key={feat.title} direction="up" delay={feat.delay}>
              <div className="bg-[#F7EDE2] border border-[#D6E8D2] rounded-xl p-5 hover:border-[#3A8A5C] hover:shadow-md transition-all duration-300 group h-full">
                ...
                <div className="font-bold text-[#14532D] text-sm mb-2">{feat.title}</div>
        ...
        <ScrollReveal direction="up" delay={100}>
          <div className="bg-gradient-to-r from-[#14532D] to-[#14532D] rounded-2xl p-6 text-center shadow-xl relative overflow-hidden">
