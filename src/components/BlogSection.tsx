...
const posts = [
  {
    ...
    color: '#D6E8D2',
    textColor: '#06402B',
    gradient: 'from-[#06402B] to-[#06402B]',
  },
  {
    ...
    color: '#FEF3C7',
    textColor: '#92400E',
    gradient: 'from-[#06402B] to-[#3A8A5C]',
  },
  {
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Scroll3DReveal direction="up">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-12 gap-4">
            <div>
              <span className="inline-flex items-center gap-2 bg-[#D6E8D2] text-[#06402B] text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-wider mb-3">
                <span className="w-2 h-2 bg-[#3A8A5C] rounded-full animate-pulse" />
                Latest Insights
              </span>
              <h2 className="text-3xl sm:text-4xl font-extrabold text-[#06402B]">Sustainability Intelligence</h2>
            </div>
        ...
        <div className="grid md:grid-cols-3 gap-7">
          {posts.map((post, i) => (
            <Scroll3DReveal key={post.slug} direction="up" delay={i * 100}>
                ...
                <Link href={`/blog/${post.slug}`} className="service-card overflow-hidden group block h-full flex flex-col">
                  ...
                  <div className={`h-44 bg-gradient-to-br ${post.gradient} relative overflow-hidden flex-shrink-0`}>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-9xl opacity-10 group-hover:opacity-20 group-hover:scale-110 transition-all duration-500">🌿</div>
                    </div>
                    <span
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    <h3 className="font-bold text-[#06402B] text-base leading-snug mb-3 line-clamp-2 flex-1">{post.title}</h3>
                    ...
                    <div className="flex items-center justify-between text-xs text-[#3A8A5C] pt-3 border-t border-[#D6E8D2]">
                      <span>{post.date}</span>
                      <span className="flex items-center gap-1 font-semibold group-hover:text-[#06402B] transition-colors">
                        Read more
