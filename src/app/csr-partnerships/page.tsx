...
// Direction pattern for 3-col grids
    ...
    <main className="pt-20">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="grid grid-cols-2 gap-4">
              {heroFeatures.map((f, i) => (
                <Scroll3DReveal key={f.title} direction={i % 2 === 0 ? 'left' : 'right'} delay={i * 80}>
                  <TiltCard intensity={8} className="h-full">
                    <div className="bg-white/10 border border-white/20 rounded-2xl p-5 text-center h-full">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <Scroll3DReveal direction="up">
              <h2 className="text-3xl sm:text-4xl font-extrabold text-[#06402B] mb-4">Why Partner With Us?</h2>
            </Scroll3DReveal>
            <Scroll3DReveal direction="up" delay={80}>
              <p className="text-[#06402B] text-lg max-w-2xl mx-auto">Unlike traditional CSR where billions are spent with zero accountability, My Green Mark provides complete transparency from sponsorship to impact.</p>
            </Scroll3DReveal>
          </div>
      ...
      <section id="packages" className="py-20 bg-[#F7EDE2]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <Scroll3DReveal direction="up">
              <h2 className="text-3xl sm:text-4xl font-extrabold text-[#06402B] mb-4">CSR Partnership Packages</h2>
            </Scroll3DReveal>
          </div>
          <div className="grid md:grid-cols-3 gap-7">
            {packages.map((pkg, i) => (
              <Scroll3DReveal key={pkg.name} direction={threeColDirections[i]} delay={i * 80}>
                <TiltCard intensity={8} className="h-full">
                  <div className={`rounded-2xl p-8 h-full ${pkg.highlight ? 'shadow-2xl scale-105' : 'border border-[#D6E8D2]'}`} style={{ background: pkg.color }}>
                    ...
                    <div className="text-3xl font-extrabold mb-5" style={{ color: pkg.highlight ? '#F4B942' : '#06402B' }}>{pkg.price}</div>
                    <ul className="space-y-2 mb-6">
                      {pkg.features.map((f) => (
                        <li key={f} className="flex items-center gap-2 text-sm" style={{ color: pkg.highlight ? '#D6E8D2' : '#06402B' }}>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            <Scroll3DReveal direction="up">
              <h2 className="text-2xl font-extrabold text-[#06402B] mb-4">Our Corporate Partners</h2>
            </Scroll3DReveal>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {partners.map((p, i) => (
              <Scroll3DReveal key={p.name} direction={threeColDirections[i % 3]} delay={i * 80}>
                <TiltCard intensity={8} className="h-full">
                  <div className="text-center p-5 bg-[#F7EDE2] border border-[#D6E8D2] rounded-xl h-full">
                    <div className="font-bold text-[#06402B] text-sm mb-1">{p.name}</div>
