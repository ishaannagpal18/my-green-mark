...
const kpis = [
  { label: 'Trees Planted', value: '42,500+', icon: '🌳', change: '+3,200 this month', color: '#14532D' },
  { label: 'CO₂ Offset', value: '8,200 T', icon: '🌍', change: '+680 T this month', color: '#14532D' },
  { label: 'Farmers Benefited', value: '1,240', icon: '👨‍🌾', change: '+87 this quarter', color: '#3A8A5C' },
  { label: 'ESG Reports Issued', value: '312', icon: '📊', change: '+24 this month', color: '#14532D' },
  { label: 'Carbon Credits', value: '4,800', icon: '♻️', change: '320 available for sale', color: '#14532D' },
]
      ...
      <section className="py-12 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-5">
            {kpis.map((kpi) => (
              <div key={kpi.label} className="stats-card rounded-2xl p-6 text-center">
                ...
                <div className="text-[#14532D] font-semibold text-sm mb-1">{kpi.label}</div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="bg-white rounded-2xl border border-[#D6E8D2] p-8">
              <h3 className="font-bold text-[#14532D] text-lg mb-6">Monthly Tree Plantation (2024)</h3>
              <div className="flex items-end gap-2 h-48 mb-3">
                {monthlyData.map((v, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    <div
                      className="w-full rounded-t-lg transition-all hover:opacity-80"
                      style={{ height: `${(v / maxValue) * 100}%`, background: `linear-gradient(180deg, #3A8A5C, #14532D)` }}
            ...
            {/* Line chart */}
            <div className="bg-white rounded-2xl border border-[#D6E8D2] p-8">
              <h3 className="font-bold text-[#14532D] text-lg mb-6">Cumulative CO₂ Offset (Tonnes)</h3>
              <div className="relative h-48 mb-3">
                <svg viewBox="0 0 300 120" className="w-full h-full" preserveAspectRatio="none">
                  ...
                  <polyline points="0,105 50,82.5 100,57 150,33 200,18 250,7.5 300,3" fill="none" stroke="#14532D" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                  {[{x:0,y:105},{x:50,y:82.5},{x:100,y:57},{x:150,y:33},{x:200,y:18},{x:250,y:7.5},{x:300,y:3}].map((pt, i) => (
                    <circle key={i} cx={pt.x} cy={pt.y} r="3" fill="#F4B942" stroke="#14532D" strokeWidth="1.5"/>
                  ))}
                </svg>
              </div>
              <div className="flex justify-between text-xs text-[#3A8A5C]">
          ...
          <div className="grid lg:grid-cols-3 gap-6 mb-8">
            <div className="bg-white rounded-2xl border border-[#D6E8D2] p-7 lg:col-span-2">
              <h3 className="font-bold text-[#14532D] text-lg mb-5">ESG Dimension Progress</h3>
              <div className="space-y-5">
                {[
                  { label: 'Environmental', score: 78, target: 85, color: '#3A8A5C' },
                  { label: 'Social', score: 82, target: 90, color: '#F4B942' },
                  { label: 'Governance', score: 88, target: 95, color: '#14532D' },
                  { label: 'Overall ESG', score: 82, target: 90, color: '#3A8A5C' },
                ].map((d) => (
                  <div key={d.label}>
                    <div className="flex justify-between text-sm mb-2">
                      <span className="font-semibold text-[#14532D]">{d.label}</span>
            ...
            <div className="bg-white rounded-2xl border border-[#D6E8D2] p-7">
              <h3 className="font-bold text-[#14532D] text-lg mb-5">SDG Alignment</h3>
          ...
          {/* State breakdown */}
          <div className="bg-white rounded-2xl border border-[#D6E8D2] p-8">
            <h3 className="font-bold text-[#14532D] text-lg mb-6">State-wise Impact</h3>
                ...
                <tbody className="divide-y divide-[#D6E8D2]">
                  {stateData.map((row) => (
                    <tr key={row.state} className="hover:bg-[#F7EDE2] transition-colors">
                      <td className="px-4 py-3 font-semibold text-[#14532D] text-sm">{row.state}</td>
                      <td className="px-4 py-3 text-[#14532D] text-sm">{row.trees.toLocaleString('en-IN')}</td>
                      <td className="px-4 py-3 text-[#14532D] text-sm">{row.co2}</td>
                      <td className="px-4 py-3 text-[#14532D] text-sm">{row.farmers}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-2 bg-[#D6E8D2] rounded-full overflow-hidden">
                            <div className="h-full bg-[#14532D] rounded-full" style={{ width: `${Math.round((row.trees / 42500) * 100)}%` }}></div>
                          </div>
